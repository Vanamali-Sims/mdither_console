...
class TestSwipes:
    ...
    def test_rearm_after_release_and_cooldown(self) -> None:
        ...
        assert recognizer.update(state(0.3, velocity=(0.1, 0.0), centroid=(0.5, 0.5))) == []
        assert run_stroke(
            recognizer,
            [
                state(0.55, velocity=(2.0, 0.0), centroid=(0.1, 0.5)),
                *stroke_states(
                    (2.0, 0.0), axis="x", sign=1, start=0.1, steps=6, time_offset=0.6
                ),
            ],
        ) == [Event.SWIPE_RIGHT]
    ...
    def test_debounce_suppresses_rapid_repeat(self) -> None:
        ...
        assert recognizer.update(state(0.2, velocity=(0.0, 0.0), centroid=(0.5, 0.5))) == []
        assert run_stroke(
            recognizer,
...
class TestOppositeLockout:
    def test_opposite_direction_suppressed_inside_lockout(self) -> None:
        ...
        recognizer.update(state(0.3, velocity=(0.0, 0.0), centroid=(0.5, 0.5)))
        assert run_stroke(
    ...
    def test_opposite_allowed_after_lockout(self) -> None:
        ...
        assert run_stroke(
            recognizer,
            stroke_states(
                (-2.0, 0.0), axis="x", sign=-1, start=0.1, steps=6, time_offset=1.3
            ),
        ) == [Event.SWIPE_LEFT]
    ...
    def test_same_direction_allowed_after_cooldown(self) -> None:
        ...
        recognizer.update(state(0.3, velocity=(0.0, 0.0), centroid=(0.1, 0.5)))
        assert run_stroke(
            recognizer,
            stroke_states(
                (2.0, 0.0), axis="x", sign=1, start=0.1, steps=6, time_offset=0.6
    ...
    def test_orthogonal_allowed_after_cooldown(self) -> None:
        ...
        recognizer.update(state(0.3, velocity=(0.0, 0.0), centroid=(0.5, 0.1)))
        assert Event.SWIPE_DOWN in run_stroke(
            recognizer,
            stroke_states(
                (0.0, 2.0), axis="y", sign=1, start=0.1, steps=6, time_offset=0.6
            ),
        )
...
class TestDoubleSwipeLeft:
    def test_second_left_within_window_becomes_back(self) -> None:
        ...
        assert run_stroke(
            recognizer,
            stroke_states(
                (-2.0, 0.0), axis="x", sign=-1, start=0.6, time_offset=0.6
            ),
    ...
    def test_slow_second_left_is_plain_swipe(self) -> None:
        ...
        assert run_stroke(
            recognizer,
            stroke_states(
                (-2.0, 0.0), axis="x", sign=-1, start=0.1, steps=6, time_offset=2.0
            ),
        ) == [Event.SWIPE_LEFT]
    ...
    def test_window_resets_after_double(self) -> None:
        ...
        recognizer.update(state(0.7, velocity=(0.0, 0.0), centroid=(0.5, 0.5)))
        assert run_stroke(
            recognizer,
            stroke_states(
                (-2.0, 0.0), axis="x", sign=-1, start=0.6, time_offset=1.2
            ),
        ) == [Event.SWIPE_LEFT]
