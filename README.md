...
object — and the UI is navigated purely through motion: swipe to step through a menu,
push toward the camera to select.
...
- **Control** — the dominant moving blob's centroid, energy, and velocity become a
...
| Input | Effect |
| --- | --- |
| Move hand | Analog cursor follows the motion centroid |
| Swipe left / right / up / down | Step the menu selection |
| Double swipe left | Back (pop one menu level) |
| Push toward camera | Select the highlighted item |
| `d` key | Toggle dither algorithm (Bayer / Floyd–Steinberg) |
| `q` or `Esc` | Quit |

The HUD shows motion energy, cursor position, the last gesture event, and FPS.
